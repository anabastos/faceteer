...
const getPostData = async (page, postsIds) => {
  const posts = postsIds && postsIds.map(async (id, index) => {
    if (typeof id === 'string') {
...
const getMembersData = async (browser, groupUrl = '') => {
  const page = browser.page;
  const membersUrl = groupUrl.length > 0
    ? `${groupUrl}/members/`
    : `${url}/groups/${browser.data.groupId}/members/`;
  await page.goto(membersUrl, { waitUntil: 'load' });
...
const getGroupData = async (browser) => {
  ...
  // TODO: Click to open all comment responses
    await clickAllButtons(page, postSelectors.moreCommentsButton);
    groupPostIds = await page.evaluate((selector) => {
      const posts = document.querySelectorAll(selector);
      return Array.prototype.map.call(posts, e => e.getAttribute('id') && e.getAttribute('id').slice(10, -4));
    }, groupSelectors.postSelector);
    ...
    posts = await getPostData(page, groupPostIds);
  } catch (e) {
    error(e);
    throw e;
  }
