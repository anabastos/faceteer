...
const getGroupData = async (browser) => {
  ...
  const getIdOfSelector = item => item && item.__FB_STORE && JSON.parse(item.__FB_STORE['data-ft'])['mf_story_key'];
  ...
  if (typeof postArticleIds === 'undefined' || postArticleIds.length === 0) {
